...
namespace
{
static constexpr const char* kDefaultService = "__default__";
}

userver::storages::postgres::Query ConfigCachePolicy::kQuery = userver::storages::postgres::Query(
...
std::vector<ConfigCacheContainer::ConfigPtr> ConfigCacheContainer::FindConfigsByService(std::string_view service) const
{
...
size_t ConfigCacheContainer::size() const
{
    return configs_to_key_.size();
}
...
ConfigCacheContainer::ConfigPtr ConfigCacheContainer::FindConfig(const ConfigCacheContainer::Key& key) const
{
    ...
    return userver::utils::FindOrDefault(configs_to_key_, ConfigCacheContainer::Key{{kDefaultService}, key.config_name}, nullptr);
}

std::vector<ConfigCacheContainer::ConfigPtr> ConfigCacheContainer::FindConfigs(std::string_view service,
                                                                               const std::vector<std::string>& ids) const
{
    ...
    for (const auto& id : ids)
    {
        ...
        if (auto c_ptr = userver::utils::FindOrDefault(configs_to_key_, ConfigCacheContainer::Key{{kDefaultService}, id}, nullptr);
            c_ptr)
        {
            result.emplace_back(c_ptr);
        }
